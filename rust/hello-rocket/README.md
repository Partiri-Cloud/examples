...
A minimal [Rocket](https://rocket.rs) application ready to deploy on [Partiri](https://partiri.cloud).
...
2. In the [Partiri dashboard](https://partiri.cloud), click **Create service** and fill in:
   ...
   | Branch | `main` |
   | Root directory | `rust/hello-rocket` |
   | Build command | `cargo build --release` |
   | Run command | `./target/release/hello-rocket` |
...
1. Build and push the image to your own registry:
   ```bash
   docker build -t ghcr.io/<you>/hello-rocket:latest .
   docker push ghcr.io/<you>/hello-rocket:latest
   ```
2. In the Partiri dashboard, click **Create service** and fill in:
   ...
   | Registry URL | `ghcr.io/<you>/hello-rocket:latest` |
...
> No `Port` field is needed — the platform injects `PORT=10000` and this example listens on it. The example reads `PORT` directly — no `ROCKET_PORT` forwarding required.
>
