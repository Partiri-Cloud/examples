...
A minimal Astro application ready to deploy on [Partiri](https://partiri.cloud).
...
2. In the [Partiri dashboard](https://partiri.cloud), click **Create service** and fill in:
   ...
   | Branch | `main` |
   | Root directory | `node/hello-astro` |
...
1. Build and push the image to your own registry:
   ```bash
   docker build -t ghcr.io/<you>/hello-astro:latest .
   docker push ghcr.io/<you>/hello-astro:latest
   ```
2. In the Partiri dashboard, click **Create service** and fill in:
   ...
   | Registry URL | `ghcr.io/<you>/hello-astro:latest` |
...
The dev server runs on Astro's default port (4321). For production parity, run the build which listens on `PORT` (default **10000**):
